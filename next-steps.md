...
*   **The Logic**: When a user taps a key button in Kotlin, you grab the connection to whatever app they are typing in using `currentInputConnection.commitText("a", 1)` to send the letter "a". You'll also need to re-implement the auto-capitalization and space-to-period replacement logic natively using the `currentInputConnection.getTextBeforeCursor` API.
...
*   **Settings & State**: To save features like custom App Shortcuts lists, themes, sizes, predictive features (on/off), text formatting options, sound, haptic feedback, and learned words, you should use `Jetpack DataStore` (Modern replacement for SharedPreferences) so changes persist after the keyboard is closed.
*   **Feedback Integration**: Android has built-in APIs for keyboard clicks (`AudioManager.playSoundEffect`) and haptics (`HapticFeedbackConstants`) which should replace the current web audio and vibration implementation.
